...
def createListOfChannels(index):
    ...
    for channel in j['_source']['channels']:
        item = """{
    "info": {
        "plot": "%s Viewers:%s  Language:%s  Mature:%s Partner:%s  Delay:%s  ",
        "plotoutline": "%s",
        "mediatype": "video",
        "tagline": "%s"
    },
    ...
    "context_menu": ["Play (Choose Quality)", "RunPlugin(plugin://plugin.video.speedrunslive/playLive/%s/-1/)"],
    "path": "plugin://plugin.video.speedrunslive/playLive/%s/-2/",
    "icon": "%s",
    "thumbnail": "%s",
    "is_playable": "True",
    "label": "%s"
}
    """    % (channel['title'], channel['current_viewers'], 'en', 'false', 'false', '0', channel['title'], channel['title'], channel['image'], channel['image'], channel['image'], channel['user_name'], channel['user_name'], channel['image'], channel['image'], channel['meta_game']+" - "+channel['title']))
